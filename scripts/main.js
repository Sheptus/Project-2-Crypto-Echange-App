//global variables - initialize empty array of coins & empty of array of btns
let modalCheckedButtons = [];
let coins = [];

//get coins + img when page loaded
$(window).on('load', function () {
    getCoins();
});

//get the coins when clicked on home
...
//show first 100 coins when "coins" link is clicked
function getCoins() {
    ...
    $.ajax({
        ...
        for (let index = 0; index < 100; index++) {
            id = (coinsResults[index].id).toString();
            moreInfoData(id);
            displayMainContent(coinsResults[index]);

            for (let index = 0; index < coins.length; index++) {
                if (id === coins[index].id) {
...
function checkReportsList(id) {
    console.log('id its geting when checking -->', id);
...
function displayMainContent(coinData, data) {
    if (data) {
        coinData = data;
    }
    ...
    let html = `<div class="col-lg-4 col-md-4 col-xs-12 col-sm-4"> 
        <div id="coin">
                <div class="coin-text">
                    <div class="row">
                        <div id="symbol" class="col-md-4">${coinData.symbol}</div>
                        <div class="col-md-5"></div>
                        <div class="col-md-3">
                            <label class="switch">
                            <input class="toggle toggle-class ${coinData.id}" type="checkbox" data-id="${coinData.id}" data-name="${coinData.name}" data-symbol=${coinData.symbol}>
                            <span class="slider round"></span>
                            </label>
                        </div>
                    </div>
                    <div id="name">${coinData.name}</div>
                </div>
            <div>
                <button id="more-info-btn" class="${coinData.id}" onClick="showMore(this)" data-id="${coinData.id}" aria-expanded="false" aria-controls="collapseExample">More Info</button>
...
function searchForCoin() {
    let searchedCoin = '';
    $.ajax('https://api.coingecko.com/api/v3/coins').done(function (data) {
        for (let i = 0; i < 1000; i++) {
            if (data[i].symbol == $("#search-input").val()) {
                searchedCoin = data[i].id;
                break;
            }
        }
        if (searchedCoin !== '') {
...
//display the graph
function displayGraphf(dataPoints, dataPoints2, dataPoints3, dataPoints4, dataPoints5) {
